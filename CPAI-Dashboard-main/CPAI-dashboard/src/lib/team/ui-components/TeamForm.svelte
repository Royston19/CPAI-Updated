<script lang="ts">
    ...
    import { showSuccessToast } from "$lib/events/utils/events-utils";
    import type { ITeam } from "../models/ITeam";
    import { FirebaseTeamRepo } from "../repo/FirebaseTeamRepo";
    import { showErrorToast } from "../utils/teams-utils";
    import ImageUploader from "./ImageUploader.svelte";
    export let editCase: boolean = false;
    export let TeamObj: ITeam = {
        id: "",
        title: "",
        subDes: "",
        image: "",
    ...
    const handleSave = async () => {
        let repo = new FirebaseTeamRepo();
        loading = true;

        if (isFile(TeamObj.image)) {
            TeamObj.image = await repo.imageToUrl(TeamObj.title, TeamObj.image as unknown as File);
        }

        try {
            if (editCase) {
                await repo.updateTeam(TeamObj);
            } else {
                await repo.saveTeam(TeamObj);
            }
            showSuccessToast(`Team ${editCase ? "Updated" : "Saved"} Successfully!`);
            goto("/team/list");
        } catch (e) {
            console.log(e);
...
<div class="my-5 pl-5">
    <h3 class="text-2xl text-gray-800 font-bold">Add Team</h3>
</div>
<div class="max-w-md mx-auto mt-14">
    ...
    <ImageUploader bind:imageUrl={TeamObj.image}></ImageUploader>
